...
export class AmocrmV4ApiService implements AmocrmV4ApiMethod {
  ...
  public async createTask<T>(amocrmRequestData: AmocrmAddTasks): Promise<IAPIResponse<T>> {
    return await this.amocrmV4RequestService.amocrmRequest<T>(AmocrmAPIV4.tasks, amocrmRequestData);
  }

  public async sendCallInfo<T>(amocrmRequestData: AmocrmAddCallInfo): Promise<IAPIResponse<T>> {
    return await this.amocrmV4RequestService.amocrmRequest<T>(AmocrmAPIV4.call, amocrmRequestData);
  }

  public async searchContact<T>(data: AmocrmGetContactsRequest): Promise<IAPIResponse<T>> {
    return await this.amocrmV4RequestService.amocrmRequest<T>(AmocrmAPIV4.contacts, data);
  }

  public async createContact<T>(amocrmRequestData: AmocrmCreateContact): Promise<IAPIResponse<T>> {
    return await this.amocrmV4RequestService.amocrmRequest<T>(AmocrmAPIV4.contacts, amocrmRequestData);
  }

  public async createLeads<T>(amocrmRequestData: AmocrmCreateLead): Promise<IAPIResponse<T>> {
    return await this.amocrmV4RequestService.amocrmRequest<T>(AmocrmAPIV4.leads, amocrmRequestData);
  }
}
