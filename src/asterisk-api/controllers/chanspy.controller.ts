...
export class ChanspyApiController {
    ...
    @ApiBearerAuth()
    @ApiOperation({
        summary: 'Получить актуальный пароль для прослушки chanSpy'
    })
    @ApiOkResponse({
    ...
    @ApiBearerAuth()
    @ApiOperation({
        summary: 'Изменить пароль для прослушки chanSpy'
    })
    @ApiBody({
        type: ChanspyDto
    })
    ...
    @ApiBearerAuth()
    @ApiOperation({
        summary: 'Сгенерировать новый пароль для прослушки chanSpy'
    })
    @ApiOkResponse({
        status: HttpStatus.OK,
