...
@Injectable()
export class BlindTransferEventParser
  implements AsteriskAmiEventProviderInterface
{
  constructor(
  ...
  private async parseBlindTransferEvent(event: AsteriskBlindTransferEvent) {
    try {
      if (
        !!event.extension &&
        event.extension.toString().length == 3 &&
        event.transfererconnectedlinenum.toString().length >= 10
      ) {
        const resultSearchId = await this.amocrmUsers.getAmocrmUser(
          event.extension,
        );
        !!resultSearchId[0]?.amocrmId
          ? await this.amocrmApi.incomingCallEvent(
              event.transfererconnectedlinenum,
              String(resultSearchId[0]?.amocrmId),
            )
          : '';
      }
    } catch (e) {
