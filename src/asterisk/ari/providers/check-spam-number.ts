...
import { AsteriskApiCheckNumberSpamData } from '@app/asterisk-api/interfaces/asterisk-api.interfaces';
...
export class CheckSpamNumberAriCall implements AsteriskAriCall {
  ...
  async getOriginateInfo(data: AsteriskApiCheckNumberSpamData): Promise<AsteriskAriOriginate> {
    try {
  ...
  private getActiveNumber(data: AsteriskApiCheckNumberSpamData, operatorInfo: Operators): NumbersInfo {
  ...
  private async getOperatorInfo(data: AsteriskApiCheckNumberSpamData): Promise<Operators> {
