...
export class PbxCallRoutingService {
    ...
    public async getExtensionRouteInfo(extension: string): Promise<ExtensionRouteInfo> {
        try {
            ...
            const extInfo = await this.pbxCallRoutingModelService.getPbxCallRouting({
                localExtension: extension
            }, EXTENSION_ROUTE_PROJ);

            if (extInfo == null) throw new HttpException({
                message: `Добавочный номер ${extension} не найден`
    ...
    public async updateGroupRoute(data: UpdateGroupRouteDTO): Promise<void> {
        try {
    ...
    public async deleteExtensionRoute(extension: string) {
        try {
            ...
            return await this.pbxCallRoutingModelService.delete({
                localExtension: extension
            });
