...
export class RoutingInfoService {
    ...
    private async getDefaultRoute(pbxCallData: PbxCallData): Promise<RoutingData> {
        ...
        const {
            dstNumber,
            callerId
        } = this.formatNumbers(operator, pbxCallData.externalNumber, operator.numbers[0].callerId);
    ...
    private getRoundRobinRoutingData(data: AggregateCallData): RoutingData {
        ...
        const {
            dstNumber,
            callerId
        } = this.formatNumbers(data.operatorInfo, data.pbxCallData.externalNumber, randomNumberInfo.callerId);
