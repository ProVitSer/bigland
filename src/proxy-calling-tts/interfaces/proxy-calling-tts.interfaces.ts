...
export class ListVoicesData {
    ...
    @ApiProperty({
        type: [String],
        description: 'Задание амплуа',
        example: '["flirt","funny","sad"]'
    })
    emotions: string[];
}
...
export class TTSFile {
    @ApiProperty({
        ...
        example: '64d1338f1364c0f30d949326'
    })
    fileId: string;
}
...
export class TTS {
    @ApiProperty({
        type: 'string',
        description: 'Текст который надо озвучить',
        example: 'Обзвон для информирования'
    })
    tts: string;
    ...
    @ApiProperty({
        type: 'string',
        required: false,
        description: 'Эмоция с которой будет озвучен текст. Если парамертр не передан, выставляется default эмоция для данного провайдера озвучки',
        example: 'good',
    })
    emotion ? : string;
}
...
export class CallingTTSTask {
    ...
    @ApiProperty({
        type: [String],
        description: 'Номера в формате E164',
        example: ['74951234567', '79101234567']
    })
    phones: string[];
...
export class CallingTaskModifyResult {
    @ApiProperty({
        type: 'boolean',
        description: 'Резульаь изменения задачи на обзвон',
        example: 'true'
    })
    result: boolean;
}
...
export class CallingTaskUpdateVoiceFile extends ApplicationId {
    @ApiProperty({
        ...
        example: '64d21e2f6837233d77a11640'
    })
    fileId: string;
}