...
export class SpamApiService {
    ...
    public async getSpamResultById(applicationId: string): Promise<SpamReportsResponseStruct[]> {
        try {
            ...
            if (result == null) throw new HttpException({
                message: `По данному ID ${applicationId} ничего не найдено`
    ...
    public async getSpamReport(date: string): Promise<SpamReportsResponseStruct[]> {
        try {
            ...
            if (result.length == 0) throw new HttpException({
                message: `По дате ${date} ничего не найдено`
            }, HttpStatus.NOT_FOUND);
    ...
    public async stopCheck(applicationId: string): Promise<StopCheckResult> {
        try {
            ...
            if (reuslt == null) throw new HttpException({
                message: `По данному ID ${applicationId} нет проверок`
            }, HttpStatus.NOT_FOUND);
    ...
    private async _stopCheck(applicationId: string, reuslt: Spam): Promise<void> {
        ...
        await this.spamModelService.update(applicationId, {
            status: ApplicationApiActionStatus.cancel
        });
    ...
    private async stopAllSpamCheck(applicationIds: string[]): Promise<void> {
        ...
        for (const id of applicationIds) {
            ...
            await this.spamModelService.update(id, {
                status: ApplicationApiActionStatus.cancel
            });
    ...
    private async _checkNumber(data: CheckNumberSpamData): Promise<void> {
        try {
